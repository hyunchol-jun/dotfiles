...
This checklist is **`$AC`** — the contract for the whole run: it scopes Phase 1, the reviewer ticks it off in Phase 2, and convergence is illegal while any criterion is unmet. Persist it to `$TMP-ac.md` and paste its exact contents into every subagent prompt (each reviewer spawn, each `SendMessage`).
