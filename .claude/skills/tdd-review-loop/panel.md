...
**Shared-tree caveat:** the panel shares one working tree with no harness-level write lock — "read-only" is an instruction a panelist may still violate — and concurrent suite runs collide on non-namespaced artifacts (`.pytest_cache`, coverage files, `__pycache__`, snapshots, `dist`/`build`/`target`, test DBs, fixed ports), producing flaky failures a panelist would misreport as real findings. So **no** panelist runs the suite — tell each explicitly: they review statically, against the green suite output captured at the Phase-1 handoff (the post-merge reviewer, with the tree to itself, runs any targeted tests serially; the final gate re-runs the full suite). The read-only evidence gate means this does not cap their severity.
